            ...
            case FRAM_TYPE_DATA :    
                ...
                {                      
                    ...
                    if(PcCmdRxId == rxId)  {  // duplicated cmd
                        ...
                        d_err       = CmdDn(pPcCmd->Data,pPcCmd->DataLen); // cmd pzrsing                        
                        SendDak(cmdtxId) ;
                        cmdtxId += 0x40 ;
                        if(d_err == 0x40)  {
    ...
    switch(pPcCmdData->CmdType)
    {
        ...
        case MCM_CMD_HEAD :  
            ...
            switch(pCodeCmd->CmdId)  {
                ...
                case MCM_SET_SSC_CMD :        //  Set_ssc(addr,mode,"H","16K")               
                    printf("\r\n Got command to re-enumerate USB...MCM_SET_SSC_CMD");
                    I2s_Init( pCodeCmd->CmdDat.SetSsc.MODE,
                              pCodeCmd->CmdDat.SetSsc.Brt,
                              pCodeCmd->CmdDat.SetSsc.Host
                             ) ;

                    FramRat = pCodeCmd->CmdDat.SetSsc.Brt ;
